...
foreach ($json as $key => $value) {
    ...
    echo '<tr onclick="post2(\'showpin.php\', \'id\',\''.$id.'\', \'key\', \''.$key.'\')"><td>'.$req["name"].'</td><td>'.$req["restaurant"].'</td><td>'.$req["note"].'</td><td>('.substr($req["phone"],1,3).') '.substr($req["phone"],4,3)."-".substr($req["phone"],7,4).'</td><td>'.$req["address"].'</td><td>'.$req["time"].'</td><tr>';
    }
echo '</table>';
?>


<input class="account-button1" type="button" margin-right="20px" onclick="post('add.php', 'id', '<?php echo $id ?>', 'fname', '<?php echo $name ?>')" value="Add an Order!"><input class="account-button2" type="button" onclick="post2('opendeliveries.php', 'id', '<?php echo $id ?>', 'fname', '<?php echo $name ?>')" value="Volunteer!">
